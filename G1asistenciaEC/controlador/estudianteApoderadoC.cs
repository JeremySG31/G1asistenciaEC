...
namespace G1asistenciaEC.controlador
{
    public class EstudianteApoderadoC
    {
        public List<EstudianteApoderadoM> ObtenerTodos()
        {
            ...
            {
                conn.Open();
                string query = @"SELECT ea.id, ea.id_estudiante, ea.id_apoderado, ea.parentesco, 
                               ...
                               INNER JOIN usuarios ue ON e.id_usuario = ue.id
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        lista.Add(new EstudianteApoderadoM
                        {
                            ...
                            NombreEstudiante = reader["nombre_estudiante"].ToString(),
        ...
        public void Insertar(EstudianteApoderadoM modelo)
        {
            using (SqlConnection conn = Conexion.ObtenerConexion())
            {
                ...
                using (var cmd = new SqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@id_estudiante", modelo.IdEstudiante);
